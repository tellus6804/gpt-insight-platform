...
if (name === "반복률") {
  if (score >= 90) {
    return `GPT Insight는 중복 단어 비율이 낮아 응답이 일관되고 매끄럽게 구성된 것으로 분석하였습니다.\n이에 따라 반복률 항목은 ${score}점으로 매우 우수하게 평가되었습니다.`;
  } else if (score >= 70) {
    return `GPT Insight는 중복 단어 비율이 전반적으로 양호한 편으로 분석되었습니다.\n이에 따라 반복률 항목은 ${score}점으로 평가되었습니다.`;
  } else {
    return `GPT Insight는 동일 단어 반복 비율이 높아 응답의 다양성과 자연스러운 흐름이 제한된 것으로 판단하였습니다.\n이에 따라 반복률 항목은 ${score}점으로 낮게 평가되었습니다.`;
  }
}
  ...
  if (name === "전문성") {
    if (score >= 90) {
      return `GPT Insight는 입력된 질문과 응답에 도메인 용어나 실무 기반 표현이 풍부하게 포함되어 전문성이 매우 높다고 판단하였습니다.\n이에 따라 전문성 항목은 ${score}점으로 매우 우수하게 평가되었습니다.`;
    } else if (score >= 70) {
      return `GPT Insight는 입력된 질문과 응답에 도메인 관련 용어가 일정 부분 포함되어 있으나, 전문성 측면에서 약간의 개선 여지가 있다고 판단하였습니다.\n이에 따라 전문성 항목은 ${score}점으로 평가되었습니다.`;
    } else {
      return `GPT Insight는 입력된 질문과 응답에서 전문 용어나 실무 기반 표현이 부족하여 전문성이 낮다고 판단하였습니다.\n이에 따라 전문성 항목은 ${score}점으로 낮게 평가되었습니다.`;
    }
  }
